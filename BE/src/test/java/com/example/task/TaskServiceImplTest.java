...
class TaskServiceImplTest {
        ...
        void update_shouldPreventPendingToCompleted() {
                ...
                assertEquals("Cannot change status from BACKLOG to COMPLETED directly. Must go through IN_PROGRESS first.",
        ...
        void update_shouldPreventCompletedWithIncompleteDependencies() {
                ...
                IllegalStateException exception = assertThrows(IllegalStateException.class,
                                () -> service.update(1, newTaskDTO));
                assertEquals("Cannot mark task as COMPLETE while it has incomplete dependencies",
                                exception.getMessage());
        }
        ...
        void update_shouldAllowOnHoldStatus() {
                ...
                task.setStatus(TaskStatus.IN_PROGRESS);
                TaskDTO newTaskDTO = new TaskDTO(1, "Test Task", "Test Description", TaskStatus.ON_HOLD,
                                50, 30, 20, LocalDate.now(), null, new HashSet<>(), new HashSet<>());
